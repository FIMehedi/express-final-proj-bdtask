...
// var uploadFimg = multer({ storage: storage }).single('featured_image');
...
router.get('/', isAdmin, function (req, res, next) {
	res.render('dashboard/adminIndex', {
	...
	async function (req, res, next) {
		...
		if (!errors.isEmpty()) {
			...
			for (var i = 0; i < categories.length; i++) {
				if (
					categories[i].slug.toLowerCase().includes(slug.toLowerCase()) &&
					slug
				) {
			...
			const addCat = await new Category(newCat).save();
...
router.post(
	...
	async function (req, res, next) {
		...
		if (!errors.isEmpty()) {
			...
			const upData = await Category.updateOne({ _id: id }, updateCat);
...
router.get('/cat-del', async function (req, res, next) {
	...
	var id = req.query.id;
	const delData = await Category.deleteOne({ _id: id });
...
router.get('/login', function (req, res, next) {
	if (!req.session.user) {
		res.render('login', { title: 'User account login', user: req.session.user });
	} else {
